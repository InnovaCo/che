...
#
# Кэш для загруженных секций, сохраняет в localStorage, используя в качестве ключа url и поле header, 
# в котором сейчас передаются данные о секциях, при этом состояние не сохраняется, если в качестве значения поля method указано "post"
#
