...
define [
  ...
  Transition:: =
    ...
    #
    # Обновляет данные секций для перехода. Если новые данные совпадают с предыдущими,
    # то ничего не происходит, иначе запускается механизм обновления секций, прежние
    # секции удаляются, вместо них вставляются новые.
    #
    update: (state) ->
      isStateTheSame = no
    ...
    #
    # Переход вперед. Требует необязательный параметр to_transition, в котором должен
    # находится индекс интересующего нас перехода.
    #
    # Полезно, когда нужно перейти на несколько шагов вперед, в случае отсутсвия параметра,
    # просто происходит переход на один шаг.
    #
    # Если параметр совпадает с индексом перехода, то в очередь asynQueue добавляется инструкция,
    # вызывающая событие "pageTransition:success", которое означает, что завершилась цепочка переходов
    #
    next: (to_transition) ->
      transition = @
    ...
    #### Transition::next([to_transition])
    #
    # Переход назад. Работает аналогично Transition::next требует необязательный
    # параметр to_transition, только переходы идут в другом направлении.
    #
    prev: (to_transition) ->
      transition = @
