...
#
# Представляет собой звено для цепочки переходов, получая данные о секциях, и ссылку на предыдущий переход,
# конструтктор Transition создает объект Invoker, устанавливает ссылку на новый объект в предыдущем объекте Transition,
# и сохраняет ссылку на предыдущий Transition. Кроме того проходит по цепочке и удаляет записи, если длина цепочки превышает 10 объектов
...
define [
  ...
  Transition = (@state, last) ->
    ...
    if last?
      ...
      # (история может ветвится, но цепочка от конца к началу не имеет ветвления)
  ...
  Transition:: =
    ...
    #
    # Обновляет данные секций для перехода. Если новые данные совпадают с предыдущими, то ничего не происходит, 
    # иначе запускается механизм обновления секций, преждние секции удаляются, вместо них вставляются новые
    #
    update: (state) ->
      isStateTheSame = no
    ...
    #
    # Переход вперед. Требует необязательный параметр to_transition, в котором должен находится индекс интересующего нас перехода.
    ...
    #### Transition::next([to_transition])
    #
    # Переход назад. Работает аналогично Transition::next требует необязательный параметр to_transition, только переходы идут в другом направлении 
    #
    prev: (to_transition) ->
      transition = @
    ...
    #### Transition::undo()
    #
    # Отмена действий перехода. Помимо отката действий объекта Invoker, запускает события "transition:undo" и "transition:current:update",
    # где в "transition:current:update" передается ссылка на предыдущий переход, а в "transition:undo" на текущий
    #
    undo: () ->
      @_invoker?.undo()
    ...
    #### Transition::invoke()
    #
    # Применение действий перехода. Помимо применения действий объекта Invoker, запускает события "transition:invoked" и "transition:current:update",
    # где в "transition:current:update" и "transition:invoked" передается ссылка на текущий переход
    #
    invoke: () ->
      @_invoker?.run()
